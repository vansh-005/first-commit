...
- [x] CI/CD: `ci.yml` (PR/main validation), Amplify frontend CD, and the OIDC-authenticated, approval-gated `deploy-infra.yml` (`MemoryLayerCiStack`); verified end to end - CI, OIDC, `production` approval gate, `MemoryLayerAlarmsStack` deploy and post-deploy health check all passed (`OPERATIONS.md` §4.1)
- [x] Repository cleanup: internal coding-agent/handoff files removed
