...
/**
 * Pure, Spring-free mapping from raw Bedrock {@code Retrieve} chunks to document-centric
 * search matches: deduplication, snippet resolution, and media-timestamp resolution.
 *
 * <p>Snippet resolution deliberately does not assume undocumented modality-specific SDK
 * fields. It parses the actual {@code Retrieve} response shape confirmed against our deployed
 * BDA Knowledge Base during Phase 4/5 verification: {@code content.text} for text chunks,
...
final class SearchResultMapper {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SearchResultMapper.class);
    ...
    static String extractDocumentId(KnowledgeBaseRetrievalResult result) {
        String fromMetadata = asString(result.metadata().get("documentId"));
    ...
    static String resolveSnippet(KnowledgeBaseRetrievalResult result) {
        ...
        String description = asString(result.metadata().get("x-amz-bedrock-kb-description"));
    ...
    private static String safeFallback(KnowledgeBaseRetrievalResult result) {
        String mediaCategory = asString(result.metadata().get("mediaCategory"));
